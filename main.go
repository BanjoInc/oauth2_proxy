...
func main() {
	...
	flagSet.String("policies-file", "", "authenticate against policies via file (json)")
	...
	fmt.Printf("domains: %s , emails: %s, policies: %s\n", opts.EmailDomains, opts.AuthenticatedEmailsFile, opts.PoliciesFile)
	err := opts.Validate()
	if err != nil {
		log.Printf("%s", err)
		os.Exit(1)
	}
	validator := NewValidator(opts.EmailDomains, opts.AuthenticatedEmailsFile, opts.PoliciesFile)
