...
func (p *OAuthProxy) OAuthCallback(rw http.ResponseWriter, req *http.Request) {
	...
	fmt.Printf("check %s:%s:%s\n", req.Host, req.Method, session.Email)
	if (p.Validator(session.Email) || p.Validator(strings.ToLower(fmt.Sprintf("%s:%s:%s", req.URL.Host, req.Method, session.Email))) || p.Validator(strings.ToLower(fmt.Sprintf("%s:%s:%s", req.Host, req.Method, session.Email)))) && p.provider.ValidateGroup(session.Email) {
...
func (p *OAuthProxy) Authenticate(rw http.ResponseWriter, req *http.Request) int {
	...
	if session != nil && session.Email != "" && !p.Validator(session.Email) && !p.Validator(strings.ToLower(fmt.Sprintf("%s:%s:%s", req.URL.Host, req.Method, session.Email))) && !p.Validator(strings.ToLower(fmt.Sprintf("%s:%s:%s", req.Host, req.Method, session.Email))) {
		log.Printf("validating session: <%s>,<%s>\n", fmt.Sprintf("%s:%s:%s", req.URL.Host, req.Method, session.Email), fmt.Sprintf("%s:%s:%s", req.Host, req.Method, session.Email))
